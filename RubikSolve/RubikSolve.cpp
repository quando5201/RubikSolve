...
void animateButtons() {
	...
	if ((ceil(xscreen) == 1 || floor(xscreen) == 1) &&
		...
		if (CUBE_SIZE == 2 || CUBE_SIZE == 3 || CUBE_SIZE == 4)
...
void keyboard(unsigned char key, int x, int y) {
	...
	//Interaction when pressing the button
...
void mouse(int button, int state, int x, int y) {
	...
	if (isMousePressed == false) {
		...
		if (xscreen == 1 && yscreen == 0) {
			...
			if (bsolve.collision(glx, gly) && (CUBE_SIZE == 2 || CUBE_SIZE == 3 || CUBE_SIZE == 4)) {
...
//motion of rubik when scroll up/down
...
void displayHelp() {
	...
	printText(-3.2, -5.2, "SHIFT : Anti-clockwise Turns", 1, GLUT_STROKE_ROMAN, colorList[3], 1);

	printText(-3.5, -5.75, "2. MOUSE", 1.5, GLUT_STROKE_ROMAN, color(0.8, 0.8, 0.8), 2.5);
	printText(-3.2, -6.0, "Drag Vertically : Rotate along X-Axis", 1, GLUT_STROKE_ROMAN, colorList[5], 1);
	printText(-3.2, -6.2, "Drag Horizontally : Rotate along Y-Axis", 1, GLUT_STROKE_ROMAN, colorList[0], 1);
	printText(-3.2, -6.4, "Scroll Up/Donw : Rotate along Z-Axis", 1, GLUT_STROKE_ROMAN, colorList[1], 1);

}

//Content in About Screen
void displayAbout() {

	printText(-3.8, 6.8, "ABOUT", 3, GLUT_STROKE_ROMAN, color(1, 0.8, 0), 3);
	printText(-3.5, 6.5, "Rubik cube simulation is a program designed using OpenGL to help who love Rubik ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 6.3, "can play around with their favourite puzzle. It is a highly sophisticated program that ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	...
	printText(-3.5, 5.9, "50x50x50 with ease. The higher size of the cube can make the system process lower ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 5.7, "becauses it takes more memory. With its responsive UI, 3D buttons and smooth ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 5.5, "transitions, it makes it easy for any user to interact with the program. We hope ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	...
	printText(-3.8, 3.7, "QN Team", 1.2, GLUT_STROKE_MONO_ROMAN, colorList[4], 2);
...
void display() {
	...
	if ((ceil(xscreen) == 1 || floor(xscreen) == 1) &&
		...
		bback10.draw();
		if (CUBE_SIZE == 2 || CUBE_SIZE == 3 || CUBE_SIZE == 4)
			bsolve.draw(); //Show solve button when the cube is 2x2, 3x3, 4x4
		displaySolution();
		glTranslatef(8, 0, 0);
